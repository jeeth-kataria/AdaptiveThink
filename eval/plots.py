...
def kpi_table(baseline, runs):
    ...
    lines = ["| Benchmark | Baseline | Router | Delta | Target | KPI met? |",
             "|---|---|---|---|---|---|"]
    router = next((r for r in runs if r.get("tag") == "router"), runs[0] if runs else None)
    n_met = 0
    if router:
    ...
    verdict = f"\n**KPI status: {n_met}/2 required benchmarks met "
    verdict += "(PASS)**" if n_met >= 2 else "(NOT YET — need >= +5% on >= 2)**"
    return "\n".join(lines) + "\n" + verdict
